...
class SparseBayesianLearner(object):
    ...
    kernel: str or None
       Type of kernel to use [currently possible: None, 'gaussian', 'poly','hpoly','cauchy']
    ...
    def __init__(self, alpha_max = 1e+3, thresh  = 1e-5,  kernel        = None,
        ...
        self.active,self.diagA,self.gamma = 0,0,0
    ...
    def fit(self,X,Y):
        ...
        if self.kernel is not None:
           X                 = SparseBayesianLearner.kernel_estimation(X,X,self.kernel, self.scaler,
                                                                                        self.p_order)        
        # add bias term
        ...
        if self.learn_type  == "classification":
            classes = set(Y)
            assert len(classes)==2,'This implementation of RVM solves only 2 class classification'
        ...
        for i in range(self.max_iter):
            ...
            # handle case when all features are pruned
            if self.m == 0:
                warnings.warn(("Warning!!! All features were pruned increase parameter " 
                               "alpha_max.By default this implementation will use rv with "
                               "smallest prior precision"))
                               
                smallest         = np.argmin(diagA)
                active[smallest] = True
                self.m          += 1
            ...
            try:
                ...
                raise ValueError(("Non positive definite matrix, caused by value of scaler,"
                                  "try different values of scaler parameter or normalise"
                                  "inputs befor trainign RVM"))
            ...
            if self.method == "fixed-point":
                ...
                # precision of likelihood update for regression case
                if self.learn_type == "regression":
    ...
    @staticmethod
    def kernel_estimation(X,Y,kernel, scaler, p):
        '''
        ...
        '''
        assert kernel in set(['poly','gaussian','hpoly','gaussian',"cauchy"]),"Undefined kernel type"
