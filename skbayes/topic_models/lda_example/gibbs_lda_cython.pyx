...
def vectorize(X):
    ...
    '''
    cdef np.ndarray[DTYPE_t,ndim=1] words
    cdef np.ndarray[DTYPE_t,ndim=1] docs
    if issparse(X):
...
class GibbsLDA(BaseEstimator,TransformerMixin):
    ...
    def fit(self,X):
        ...
        for j in range(self.n_burnin):
             ...
             if self.compute_score:
                 self.scores_[j] = self._joint_loglike(n_d,n_vocab,n_docs,doc_topic,
                                                       word_topic,topics)
    ...
    def _gibbs_sample_lda(self,np.ndarray[DTYPE_t,ndim=1] words, np.ndarray[DTYPE_t,ndim=1] docs,
        ...
        for i in xrange(len(words)):
            ...
            logp_latent = np.log(doc_topic[di,:]+alpha) - np.log(alpha*n_topics + n_d[di]-1);
    ...
    def transform(self,X):
        ...
        wt,doc_topic,ta,ts = self._gibbs_sample_lda(words,docs,self.topic_assignment,
                                                    self.word_topic,self.doc_topic,self.topics,
                                                    n_d,n_words)
        return doc_topic
